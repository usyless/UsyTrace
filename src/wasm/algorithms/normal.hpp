...
inline void traceFor(uint32_t startX, uint32_t startY, const int step, frTrace& trace, const ImageData<4>& imageData, const uint32_t maxLineHeight, const uint32_t maxJump, RGBTools& colour) {
    ...
    yValues.reserve(maxLineHeight + maxJump * 2 + 1);
    uint32_t currJump = 0;
    const uint32_t maxHeight = imageData.height - 1;
    const auto width = imageData.width;

    for (; startX < width; startX += step) {
        yValues.clear();
        auto max = static_cast<int>((maxLineHeight + (currJump * 2)) / 2);
        auto low = (max > static_cast<int>(startY)) ? -static_cast<int>(startY) : -max;
        ...
        if (!yValues.empty()) {
            currJump = 0;
            startY = yValues[yValues.size() / 2];

            if (step > 0) {
                trace.emplace_hint(trace.end(), startX, startY);
            } else {
                trace.emplace_hint(trace.begin(), startX, startY);
            }
