...
const typeMap = {
    ...
    trace: Module.cwrap("trace", "string", ["number", "number", "number", "number", "number", "number"]),
    addImage: Module.cwrap("addImage", "", ["number", "number", "number", "number"]),
    removeImage: Module.cwrap("removeImage", "", ["number"]),
    point: Module.cwrap("point", "string", ["number", "number", "number"]),
    undo: Module.cwrap("undo", "string", ["number"]),
    clear: Module.cwrap("clear", "", ["number"]),
    auto: Module.cwrap("autoTrace", "string", ["number", "number", "number", "number"]),
    exportTrace: Module.cwrap("exportTrace", "string", ["number", "number", "number", "number", "number", "number", "number", "number", "number", "number", "number", "number", "number"]),
    snap: Module.cwrap("snap", "number", ["number", "number", "number", "number"])
};
...
// Tracing
function trace(data) {
    return defaultTraceResponse(data, api.trace(data.id, parseInt(data.x), parseInt(data.y),
        parseInt(data.maxLineHeightOffset), parseInt(data.maxJumpOffset), parseInt(data.colourTolerance)));
}

function point(data) {
    return defaultTraceResponse(data, api.point(data.id, parseInt(data.x), parseInt(data.y)));
}

function auto(data) {
    return defaultTraceResponse(data, api.auto(data.id, parseInt(data.maxLineHeightOffset),
        parseInt(data.maxJumpOffset), parseInt(data.colourTolerance)));
}

function undo(data) {
    return defaultTraceResponse(data, api.undo(data.id));
}
...
function exportTrace(data) {
    data.export = api.exportTrace(data.id, parseInt(data.PPO), data.delim === "tab" ? 1 : 0,
...
function snapLine(data) {
    data.line.pos = api.snap(data.id, parseInt(data.line.pos), data.line.dir === "x" ? 1 : 0, parseInt(data.dir));
    return data;
}